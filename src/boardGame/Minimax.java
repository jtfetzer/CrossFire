...
public class Minimax implements Runnable{
	...
	public static void min(BoardNode node, int depth){
		...
		if(node.lastMove.getBestMoveValue() == CrossFire.MAX_WINS){
			node.value = CrossFire.MAX_WINS;
			node.maxWinDepth = depth;
			return;
		}

		if(depth == CrossFire.MAX_DEPTH || node.getNumEmptySpaces() == 0){
			return;
		}
		
		MoveSet attacks = CrossFire.getMostPromisingMoves(node, CrossFire.ATTACK_MOVES);
		MoveSet blocks = CrossFire.getMostPromisingMoves(node, CrossFire.BLOCK_MOVES);
		MoveSet bestMoves = blocks.addAll(attacks).reduceMin(CrossFire.MAX_WIDTH / 2);
	...
	public static void max(BoardNode node, int depth){
		...
		if(node.lastMove.getBestMoveValue() == CrossFire.MIN_WINS){
			node.value = CrossFire.MIN_WINS;
